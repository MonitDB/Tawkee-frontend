  ...
  useEffect(() => {
    ...
    if (!token) {
      setVerificationResult({
        success: false,
        message:
          'Verification token not found in URL. Please use the link sent to your email.',
      });
      setIsLoading(false);
      return;
    }
    ...
    const verifyToken = async () => {
      ...
      try {
        const response = await fetch(
          `${env.API_URL}/auth/verify-email?token=${token}`,
          {
            method: 'GET',
            headers: {
  ...
  const renderContent = () => {
        ...
        >
          ...
          <Typography variant="body1">
            {verificationResult.message ||
              'Your account was verified and is ready for use.'}
          </Typography>
    ...
    return (
      ...
      >
        ...
        <Typography variant="body1" color="text.secondary">
          {verificationResult.message ||
            'We could not verify your account. Please check the link sent via email and try again.'}
        </Typography>
      </Stack>
    );
  };
