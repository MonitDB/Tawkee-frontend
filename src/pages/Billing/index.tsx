...
export const subscriptionColorsTypography: Record<
  SubscriptionStatus,
...
export default function Billing() {
  ...
  const canManageSubscriptionAsAdmin = can(
    'MANAGE_SUBSCRIPTION_AS_ADMIN',
    'BILLING'
  );
  const canManageSubscription = can('MANAGE_SUBSCRIPTION', 'BILLING');
  const canPurchaseExtraCreditsAsAdmin = can(
    'PURCHASE_EXTRA_CREDITS_AS_ADMIN',
    'BILLING'
  );
  const canPurchaseExtraCredits = can('PURCHASE_EXTRA_CREDITS', 'BILLING');
  const canManageSmartRechargeSettingsAsAdmin = can(
    'MANAGE_SMART_RECHARGE_SETTINGS_AS_ADMIN',
    'BILLING'
  );
  const canManageSmartRechargeSettings = can(
    'MANAGE_SMART_RECHARGE_SETTINGS',
    'BILLING'
  );
  ...
  const [workspacePlanCreditsState, setWorkspacePlanCreditsState] = useState<
    number | 'UNLIMITED'
  >(workspacePlanCredits);
  const [workspaceExtraCreditsState, setWorkspaceExtraCreditsState] =
    useState<number>(workspaceExtraCredits);
  ...
  const [autoRechargeEnabled, setAutoRechargeEnabled] = useState(
    !!smartRecharge?.active
  );
  const [autoRechargeThreshold, setRechargeThreshold] = useState(
    smartRecharge?.threshold
  );
  const [autoRechargeAmount, setAutoRechargeAmount] = useState(
    smartRecharge?.rechargeAmount
  );
  ...
  const handleUpdateSmartRechargeSetting = async (
    ...
    currentAutoRechargeEnabled: boolean
  ) => {
  ...
  useEffect(() => {
    if (!canView) {
      notify(
        'You do not have permission to view Billing details of your workspace.',
        'warning'
      );
      navigate('/');
    }
  ...
  useEffect(() => {
    const fetchBillingData = async (workspaceId: string) => {
      ...
      setWorkspacePlanCreditsState(
        response?.planCreditsRemaining as number | 'UNLIMITED'
      );
  ...
  const workspacePlanCreditsLabel =
    workspacePlanCredits == Infinity ? 'Unlimited' : workspacePlanCredits;
    ...
    <Card variant="outlined" sx={{ margin: '0 auto', width: '100%' }}>
      <CardContent>
        <Box sx={{ p: 3 }}>
            ...
            >
              ...
              {userBelongsToSelectedWorkspace
                ? subscription?.status && (
                    ...
                    >
                      <Typography variant="h5">Your subscription is</Typography>
                      <Typography
                        variant="h4"
                        component="span"
                        color={
                          subscriptionColorsTypography[
                ...
                : subscriptionState?.status && (
                    ...
                    >
                      <Typography variant="h5">This subscription is</Typography>
                      <Typography
                        variant="h4"
                        component="span"
                        color={
                          subscriptionColorsTypography[
                            subscriptionState?.status as SubscriptionStatus
                          ]
                        }
                      >
                        {
          ...
          <Typography
            variant="body2"
            sx={{ mb: 2, display: 'flex', flexDirection: 'row', gap: 2 }}
          >
            {userBelongsToSelectedWorkspace
              ? 'You currently have '
          ...
          <Card variant="outlined" sx={{ mb: 3 }}>
            <CardContent>
              ...
              {subscriptionState?.featureOverrides ? (
                <Box sx={{ mt: 1 }}>
                  {subscriptionState.featureOverrides.map(
            ...
            {subscriptionState?.status === 'TRIAL' ? (
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                <Button
                  ...
                  color="secondary"
                  onClick={() =>
                    openCustomerPortal(user?.workspaceId as string)
                  }
                >
                  Manage Subscription
                </Button>
                  ...
                  <CardContent>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      <TextField
                        ...
                        onChange={(event) =>
                          setRechargeAmount(Number(event.target.value))
                        }
                        disabled={
                          userBelongsToSelectedWorkspace
                      ...
                      />
                      <Chip
                        label={rechargeCost + ' ' + currency.toUpperCase()}
                      />
                      <Button
                        variant="contained"
                  ...
                  <CardContent>
                    ...
                    >
                      <TextField
                        ...
                        onChange={(e) =>
                          setRechargeThreshold(Number(e.target.value))
                        }
                        size="small"
                        disabled={
                          stripeLoading || !autoRechargeEnabled
                            ? true
                            : userBelongsToSelectedWorkspace
                              ? !canManageSmartRechargeSettings
                              : !canManageSmartRechargeSettingsAsAdmin
                      ...
                      <TextField
                        ...
                        onChange={(e) =>
                          setAutoRechargeAmount(Number(e.target.value))
                        }
                        size="small"
                        disabled={
                          stripeLoading || !autoRechargeEnabled
